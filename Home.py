...
class Main:
    ...
    def run(self):
        ...
        with st.form("form_data", enter_to_submit=False):
            ...
            self.work_center_name = st.text_input(
                "Input Details Below.", placeholder="Work Center Name"
            )
            ...
            # Production Quantity
            self.quantity = st.text_input(
                "", placeholder="Quantity", label_visibility="hidden"
            )
            ...
            if self.submitted:
                # Completed Form
                if (
                    self.work_center_name
                    and self.capacity
                    and self.cost_per_hour
                    and self.operation
                    and self.assigned_work_center
                    and self.time_per_unit
                    and self.quantity
                ):
                    ...
                    st.session_state.work_center_data["quantity"] = int(
                        self.quantity
                    )
        ...
        self.check: bool = (
            (len(self.dictionary["work_centers"]) == 0)
            and (len(self.dictionary["routing"]) == 0)
            and not (self.dictionary["quantity"])
        )
        ...
        if not (self.check):
            ...
            if st.session_state.save_to_json is None or st.session_state == False:
                with self.yes:
                    if st.button("yes"):
                        st.empty()
                ...
                with self.display:
                    ...
                    if st.button("Display results"):
                        if self.check:
                            st.warning(
                                "Currently, there is no data found in work_center_data.",
                                icon="⚠️",
                            )
                        else:
        ...
        if st.session_state.save_to_json == True:
            ...
            with st.form("save_json", enter_to_submit=False):
                self.file_name = st.text_input(
                    "Input below.", placeholder="File name"
                )
                ...
                if self.save_file:
                    try:
                        ...
                        self.full_json_path = json_folder / f"{self.file_name}.json"

                        with open(self.full_json_path, "w") as file_write:
                            json.dump(self.json_data, file_write, indent=4)

                    except Exception as e:
                        st.error(
                            f"Failed to save data to JSON File: {e}", icon="🚨"
                        )
                ...
                if bool((len(self.file_name) > 0)) == True:
                    self.note_1.empty()
                    self.note_2 = st.info(
                        f"Now saving {self.file_name}.json", icon="ℹ️"
                    )
                    ...
                    for p_complete in range(100):
                        ...
                        self.progbar.progress(
                            p_complete + 1, text=self.save_text
                        )
